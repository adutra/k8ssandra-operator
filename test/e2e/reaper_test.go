...
func createSingleReaper(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	dcKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "dc1"}}
	reaperKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc1-reaper"}}
	...
	checkKeyspaceExists(t, f, ctx, *k8sCtx0, namespace, "test", "test-dc1-default-sts-0", "reaper_db")

	testDeleteReaperManually(t, f, ctx, kcKey, dcKey, reaperKey)
	testRemoveReaperFromK8ssandraCluster(t, f, ctx, kcKey, dcKey, reaperKey)

	t.Log("deploying Reaper ingress routes in", *k8sCtx0)
	f.DeployReaperIngresses(t, ctx, *k8sCtx0, 0, namespace, "test-dc1-reaper-service")
	defer f.UndeployAllIngresses(t, *k8sCtx0, namespace)

	t.Run("TestReaperApi[0]", func(t *testing.T) {
		t.Log("test Reaper API in context", *k8sCtx0)
		reaperUiSecretKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-reaper-ui"}}
...
func createSingleReaperWithEncryption(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	t.Log("deploying Reaper ingress routes in context", *k8sCtx0)
	f.DeployReaperIngresses(t, ctx, *k8sCtx0, 0, namespace, "test-dc1-reaper-service")
	defer f.UndeployAllIngresses(t, *k8sCtx0, namespace)
...
func createMultiReaper(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: cqlSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: cqlSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: jmxSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: jmxSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: uiSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: uiSecretKey})
	...
	reaper1Key := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc1-reaper"}}
	reaper2Key := framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc2-reaper"}}
	stargate1Key := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc1-stargate"}}
	stargate2Key := framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc2-stargate"}}

	checkDatacenterReady(t, ctx, dc1Key, f)
	checkDatacenterReady(t, ctx, dc2Key, f)

	t.Log("check Stargate auth keyspace created in both clusters")
	checkKeyspaceExists(t, f, ctx, *k8sCtx0, namespace, "test", "test-dc1-default-sts-0", stargate.AuthKeyspace)
	checkKeyspaceExists(t, f, ctx, *k8sCtx1, namespace, "test", "test-dc2-default-sts-0", stargate.AuthKeyspace)

	t.Log("check Reaper custom keyspace created in both clusters")
	checkKeyspaceExists(t, f, ctx, *k8sCtx0, namespace, "test", "test-dc1-default-sts-0", "reaper_ks")
	checkKeyspaceExists(t, f, ctx, *k8sCtx1, namespace, "test", "test-dc2-default-sts-0", "reaper_ks")
	...
	checkNodeToolStatus(t, f, *k8sCtx0, namespace, "test-dc1-default-sts-0", 2, 0, "-u", username, "-pw", password)

	t.Log("check nodes in dc2 see nodes in dc1")
	checkNodeToolStatus(t, f, *k8sCtx1, namespace, "test-dc2-default-sts-0", 2, 0, "-u", username, "-pw", password)

	t.Log("deploying Stargate and Reaper ingress routes in both clusters")
	f.DeployReaperIngresses(t, ctx, *k8sCtx0, 0, namespace, "test-dc1-reaper-service")
	f.DeployReaperIngresses(t, ctx, *k8sCtx1, 1, namespace, "test-dc2-reaper-service")
	f.DeployStargateIngresses(t, *k8sCtx0, 0, namespace, "test-dc1-stargate-service", username, password)
	f.DeployStargateIngresses(t, *k8sCtx1, 1, namespace, "test-dc2-stargate-service", username, password)

	defer f.UndeployAllIngresses(t, *k8sCtx0, namespace)
	defer f.UndeployAllIngresses(t, *k8sCtx1, namespace)

	t.Run("TestReaperApi[0]", func(t *testing.T) {
		secretKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: uiSecretKey}
	...
	t.Run("TestReaperApi[1]", func(t *testing.T) {
		secretKey := framework.ClusterKey{K8sContext: *k8sCtx1, NamespacedName: uiSecretKey}
		username, password := retrieveCredentials(t, f, ctx, secretKey)
...
func createReaperAndDatacenter(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	dcKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "dc1"}}
	reaperKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "reaper1"}}

	checkDatacenterReady(t, ctx, dcKey, f)

	t.Log("create Reaper keyspace")
	_, err := f.ExecuteCql(ctx, *k8sCtx0, namespace, "test", "test-dc1-rack1-sts-0",
	...
	checkKeyspaceExists(t, f, ctx, *k8sCtx0, namespace, "test", "test-dc1-rack1-sts-0", "reaper_db")

	checkReaperReady(t, f, ctx, reaperKey)

	t.Log("deploying Reaper ingress routes in context", *k8sCtx0)
	f.DeployReaperIngresses(t, ctx, *k8sCtx0, 0, namespace, "reaper1-service")
	defer f.UndeployAllIngresses(t, *k8sCtx0, namespace)

	t.Run("TestReaperApi[0]", func(t *testing.T) {
		t.Log("test Reaper API in context", *k8sCtx0)
		secretKey := framework.ClusterKey{K8sContext: *k8sCtx0, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "reaper-ui-secret"}}
		username, password := retrieveCredentials(t, f, ctx, secretKey)
		testReaperApi(t, ctx, 0, "test", "reaper_db", username, password)
	})
}
